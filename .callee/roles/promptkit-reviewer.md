...
kind: Role
spec:
  description: Deeply reviews Callee changes with evidence-backed guardrails, exhaustive path tracing, security analysis, and concurrency checks.
  provider:
    type: codex
    model: gpt-5.6-sol
    reasoning: high
...
**Context**: Callee is a Go CLI for provider-aware Markdown subagent roles. Preserve the CLI-only surface, nested provider fields, protocol-agnostic thread handles, and Norma Runtime ownership of ACP process logic.
