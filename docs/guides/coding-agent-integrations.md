...
Callee's coding-agent integrations teach Codex, Claude Code, Grok Build, Copilot CLI, OpenCode, and Cursor how to inspect, create, validate, and run project-defined resources. They provide convenience and safety instructions around the same CLI; they do not change workflow semantics or supply any runtime dependency.
...
| Create Agent | Author a `Role`, `Script`, `Human`, `TypeSafeJev`, `OpenRouterDecision`, `Sequential`, `Parallel`, `Loop`, or `Router` in Markdown or YAML, optionally assemble a Role from PromptKit, validate the physical file, and resolve the complete tree. |
...
| Target | Coding-agent invocation | Integration method |
| --- | --- | --- |
...
Codex setup first removes an existing marketplace registration named `callee`, ignoring only the specific case where it is absent, then adds the repository marketplace and plugin. The other marketplace targets run their coding-agent commands directly.

## Manual marketplace setup

Use manual setup only when you need to control coding-agent installation separately from starter-resource creation.
...
| Coding-agent asset | Repository source | Project destination |
| --- | --- | --- |
...
The coding-agent skill invokes `callee agent list --json` and `callee agent view <id> --json` to understand the current project before running or authoring. Execution must still occur through `callee agent run` attached to a real controlling terminal. The coding agent should keep terminal interaction separate from stdout and stderr and treat the exit status as authoritative.
...
Configure each Role's runtime under `spec.provider`; see
[ACP provider configuration](acp-providers.md). TypeSafeJev and
OpenRouterDecision use their own service credentials and model fields; see
[Agent resources](../reference/agent-resources.md#typesafejev-and-openrouterdecision).
