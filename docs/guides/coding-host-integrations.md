...
| Create Agent | Author a `Role`, `Script`, `Human`, `TypeSafeJev`, `OpenRouterDecision`, `Sequential`, `Loop`, or `Router` in Markdown or YAML, optionally assemble a Role from PromptKit, validate the physical file, and resolve the complete tree. |
