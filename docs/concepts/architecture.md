...
Roles and DynamicRoles use ACP provider sessions through Norma Runtime. Scripts run local
...
| Concept | Meaning |
| --- | --- |
| Resource | One versioned Markdown or YAML definition with a `Role`, `DynamicRole`, `Script`, `Human`, `TypeSafeJev`, `OpenRouterDecision`, `Sequential`, `Parallel`, `Loop`, or `Router` kind. |
...
| DynamicRole | A Role-family leaf that renders and validates its provider configuration from visit-time state before lazy startup. |
...
| Role-family visit | One execution of a resolved Role or DynamicRole node, with a fresh provider session even when its provider process is reused. |
...
workflow runner + one shared ephemeral state object
        |
        +--> Role / DynamicRole -> Norma Runtime -> ACP provider process -> fresh session
        +--> Script -> local shell
...
Role, DynamicRole, Script, Human, TypeSafeJev, and OpenRouterDecision are the current native
...
A root run reuses a provider process when Roles or DynamicRoles normalize to the same effective public provider type and command. Session configuration does not change that process identity: `model`, `mode`, and `reasoning` are applied when creating the visit session. Dynamic providers are rendered and validated only when reached.

Every Role or DynamicRole visit receives a fresh provider session. Repeated visits in a Loop therefore do not continue the previous provider conversation. A REPL visit is the exception only within that single visit: `await` retains its session for the next operator turn. Provider processes remain live until the root finishes and are closed in reverse start order. A cleanup error suppresses the otherwise successful artifact.
...
A Role's static or DynamicRole's rendered `spec.provider.type` independently selects an ACP backend. For example,
...
| [`internal/runtime`](../../internal/runtime) | Callee-to-Norma provider normalization, ACP process reuse, and Role-family visit sessions. |
