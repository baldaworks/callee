...
| [`internal/doctor`](../../internal/doctor) | Static Role-provider readiness, DynamicRole deferral, evaluation configuration checks, and graph renderers. |
...
The Workflows API is a clean break. Its supported kinds are `Role`, `DynamicRole`, `Script`, `Human`, `TypeSafeJev`, `OpenRouterDecision`, `Sequential`, ADK-backed `Parallel`, `Loop`, and deterministic single-branch `Router`. Do not restore legacy `exec` or `role` commands, selector-based agent roles, thread flags, or unversioned Role resources. Do not add arbitrary graph edges, Gemini, a server transport, a Callee thread store, handle binding, or duplicated ACP process logic without an explicit product decision.

One root run owns one state object. Every Role or DynamicRole visit owns a fresh provider session. Norma Runtime remains the ACP process layer.
...
go run ./cmd/callee agent validate examples/roles/dynamic-reviewer.md
