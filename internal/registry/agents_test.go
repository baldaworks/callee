...
func TestResolveComputesInheritedSessionPolicies(t *testing.T) {
	t.Parallel()
...
`)
	...
	if phaseNode.Session != agent.SessionModeStateful || phaseNode.SessionScopeID != "workflows/loop" {
		t.Errorf("phase session = %q scope %q, want stateful scope workflows/loop", phaseNode.Session, phaseNode.SessionScopeID)
	}

	inherited := phaseNode.Children[0]
	if inherited.Session != agent.SessionModeStateful || inherited.SessionScopeID != "workflows/loop" {
		t.Errorf("inherited worker session = %q scope %q, want stateful scope workflows/loop", inherited.Session, inherited.SessionScopeID)
	}

	fresh := phaseNode.Children[1]
	if fresh.Session != agent.SessionModeFresh || fresh.SessionScopeID != "" {
		t.Errorf("fresh worker session = %q scope %q, want fresh without scope", fresh.Session, fresh.SessionScopeID)
	}
}

func TestResolveBindsExplicitNestedStatefulSessionToNearestLoop(t *testing.T) {
	t.Parallel()

	worker := decodeAgent(t, "roles/worker", roleAgent("worker", nil))
	inner := decodeAgent(t, "workflows/inner", `---
apiVersion: callee.metalagman.dev/v1alpha1
kind: Loop
spec:
  description: inner
...
{{ .Input }}
`)
	outer := decodeAgent(t, "workflows/outer", `---
apiVersion: callee.metalagman.dev/v1alpha1
kind: Loop
spec:
  description: outer
...
`)
	...
	configured, err := NewAgentRegistry([]agent.Resource{worker, inner, outer})
	if err != nil {
		t.Fatalf("NewAgentRegistry() error: %v", err)
	}

	root, err := configured.Resolve("workflows/outer")
	if err != nil {
		t.Fatalf("Resolve() error: %v", err)
	}

	innerNode := root.Children[0]
	if innerNode.SessionScopeID != "workflows/outer" {
		t.Errorf("inner inherited scope = %q, want workflows/outer", innerNode.SessionScopeID)
	}

	workerNode := innerNode.Children[0]
	if workerNode.Session != agent.SessionModeStateful || workerNode.SessionScopeID != "inner" {
		t.Errorf("worker session = %q scope %q, want stateful scope inner", workerNode.Session, workerNode.SessionScopeID)
	}
}

func TestResolveRejectsSessionPolicyWithoutLoop(t *testing.T) {
	t.Parallel()
...
`)
	...
	_, err := NewAgentRegistry([]agent.Resource{worker, pipeline})
	if err == nil {
		t.Fatal("NewAgentRegistry() error = nil, want session placement error")
	}

	for _, want := range []string{"session", "workflows/pipeline -> roles/worker", "beneath a Loop"} {
		if !strings.Contains(err.Error(), want) {
