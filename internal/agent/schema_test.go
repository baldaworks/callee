...
func TestSchemaForKindReturnsStandaloneSchema(t *testing.T) {
	...
	tests := []struct {
		...
		{kind: DynamicRoleKind, definition: "dynamicRole"},
...
func assertSchemaDefinitions(t *testing.T, kind Kind, definition string, document map[string]any) {
	...
	for _, other := range []string{"role", "dynamicRole", "script", "human", "typeSafeJev", "openRouterDecision", "sequential", "loop", "router"} {
		if other == definition {
			continue
		}
