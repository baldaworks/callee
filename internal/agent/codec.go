...
func validateDocumentDispatch(id, source string, document *yaml.Node, lineOffset int) error {
	...
	if kind == nil {
		return fmt.Errorf("agent %q: %s: missing kind; supported kinds: Role, DynamicRole, Script, Human, TypeSafeJev, OpenRouterDecision, Sequential, Parallel, Loop, Router", id, sourcePosition(source, firstValueLine, 1))
	}

	switch Kind(kind.Value) {
	case RoleKind, DynamicRoleKind, ScriptKind, HumanKind, TypeSafeJevKind, OpenRouterDecisionKind, SequentialKind, ParallelKind, LoopKind, RouterKind:
		return nil
	default:
		return fmt.Errorf("agent %q: %s: unsupported kind %q; supported kinds: Role, DynamicRole, Script, Human, TypeSafeJev, OpenRouterDecision, Sequential, Parallel, Loop, Router", id, sourcePosition(source, kind.Line+lineOffset, kind.Column), kind.Value)
	}
}
